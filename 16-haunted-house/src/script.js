...
loader.load( '/little_tree.glb', function ( gltf ) {
    ...
    tree.traverse((child) => {
        if (child.isMesh) {
            ...
            child.material.onBeforeCompile = (shader) => {
                ...
                float heightFactor = (position.y - uMinY) / (uMaxY - uMinY);
                heightFactor = clamp(heightFactor, 0.0, 1.0);

                heightFactor = pow(heightFactor, 2.0);

                float sway = sin(uTime * uWindSpeed + position.z) * uWindStrength;
    ...
    tree.position.set(-6.5, 0, -6);
    //tree.scale.setScalar(0.25);
    ...
    for(let i = 0; i < 15; i++)
    {
        ...
        clone.scale.setScalar(Math.random() * 0.2 + 0.1);
